(function(){ //self invoking function keeps all variables local
    ...
    //function isMax(a, b) {\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
    function isMax(a) {//added
        positionComparisonBox(a);
        return (myArray[a] > maxValue);//added
    }//end of isMax
    ...
    function setPhase(newPhase) {
        ...
        $("#startMaxBtn").attr("disabled", phase == ACTIVE || phase == ASLEEP );
        $("#newMaxBtn").attr("disabled", phase == ACTIVE);
    }//end setPhase

    //refill the array and reset phase so to be ready for a new search for max value.
    function newMaxSearch() {
        setPhase(AWAKE);
        confirmed = false;
        boxPosition = -1;
        movingElement = -1;
    ...
    function setIndividualElement(i) {
       var x,y;
        if (i < 10) {
            x = xOffset + (i)*60;//added
            y = textFirstRow_y ;
       }
       ctx.fillText(myArray[i], x, y);
    }//end of setIndividualElement

    function setMaxValue(){ //added function
         var x,y;
	 x = xOffset + 270;
         y = textSecondRow_y;
         ctx.fillText(maxValue, x, y);
    }//end of setMaxValue
    ...
    function draw() {
        ...
        ctx.fillText("MaxValue",xOffset-10 + 255, yOffset2+5);
    ...
    function elementSelect(destination, source) {
        ...
        xStart = xOffset + (source)*60;//added
        yStart = yOffset1 - 50;
    ...
    function display() {
        if (isMax(i)) {
            text = text + "<br>"+myArray[i] +
                    " is greater than the current maxValue so maxValue takes the value of " +
                     myArray[i]+".";
            showComparisons(text);
            elementSelect(maxValue, i);//added
        }//end of if

        i = i + 1;
        if(i==12){
            finished = true;
            text = "Search is finished and the Maximum Value in the array is " + maxValue;
    ...
    $(document).ready(function() {
        var canvas = document.getElementById("searchMaxcanvas"); // A reference to the canvas element.
        ...
        textFirstRow_y = 60;
        textSecondRow_y = 184;
        $("#startMaxBtn").on('click',start);
        $("#newMaxBtn").on('click',newFind);
